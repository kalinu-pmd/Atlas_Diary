...
function Navbar() {
  ...
  const authControls = user ? (
    <div className="flex items-center gap-3">
      ...
      <Link
        to="/create-post"
        className="hidden sm:inline-flex items-center gap-2 bg-gradient-to-b from-accent-green to-accent-green-2 text-white font-extrabold text-sm px-4 py-2 rounded-full shadow-[0_8px_30px_rgba(47,107,79,0.12)] hover:-translate-y-0.5 transition-transform no-underline whitespace-nowrap"
      >
        <MdAdd size={16} />
        <span>Post</span>
      </Link>
      ...
      <div ref={menuRef} className="relative flex items-center">
        <button
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") setUserMenuOpen((s) => !s);
          }}
        ...
        {userMenuOpen && (
          <div className="absolute right-0 top-full mt-2 flex justify-end w-max z-50">
            <div
              className="w-3 h-3 bg-white/95 border border-dark-green/5 rotate-45 -mt-2 mr-4"
              style={{ boxShadow: "rgba(6,24,15,0.05) 0px 6px 12px" }}
            />
            <div className="bg-white/95 rounded-2xl shadow-lg w-56 p-3 border border-dark-green/5 backdrop-blur-sm">
              ...
              <button
                onClick={logout}
                className="flex items-center gap-3 w-full text-left px-3 py-2 text-sm text-white bg-orange rounded-lg hover:bg-orange-hover transition-colors"
              >
                <MdExitToApp size={18} />
